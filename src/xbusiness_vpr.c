							 ...
							 const char *name)		/* 用户名 */
{
	...
	if (!json_object_get_boolean(json_object_object_get(result_json, "success"))) {
		return json_object_get_int(json_object_object_get(result_json, "error_code"));
	}
							 ...
							 const char *name)		/* 用户名 */
{
	...
	return xvpr_reserve_speeches(handle, name);
}
						...
						xvpr_person_info *info)		/* 返回说话人信息 */
{
	...
	if (sizeof(persons) <= 0) { return XVPR_CODE_PARAM_ERR; }
